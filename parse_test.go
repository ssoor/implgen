...
func TestFileParser_ParsePackage(t *testing.T) {
	...
	newP, err := p.parsePackage("github.com/ssoor/implgen/internal/tests/custom_package_name/greeter")
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
...
func checkGreeterImports(t *testing.T, imports map[string]importedPackage) {
	...
	if validatorPackage, ok := imports["validator"]; !ok {
		...
		expectedValidatorPackage := "github.com/ssoor/implgen/internal/tests/custom_package_name/validator"
	...
	if clientPackage, ok := imports["client"]; !ok {
		...
		expectedClientPackage := "github.com/ssoor/implgen/internal/tests/custom_package_name/client/v1"
